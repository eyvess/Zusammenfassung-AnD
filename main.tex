...
\subsection{Induktion}
  ...
  \end{equation}
    ...
    $\mathcal{O}(1) \in [\mathcal{O}(\frac{1}{n^2}) \in \mathcal{O}(\frac{1}{n}) \in \ ]  \mathcal{O}(\log \log n)\in \mathcal{O}(\log n) \in \mathcal{O}(\sqrt{n}) \in \mathcal{O}(n) \in \mathcal{O}(n\log n) \in \mathcal{O}(n^2) \in \mathcal{O}(2^n) \in \mathcal{O}(2^{n^{2}}) \in \mathcal{O}(n!) $
...
Zusätzlich gilt: die Tiefe eines binary tree ist gleich wie die Höhe eines binary trees.\footnote{Vergleiche mit \href{https://www.baeldung.com/cs/binary-tree-height}{weitere Beispiele \& Erklärungen}}
    ...
    \begin{algorithmic} 
        \For {$i \in \left\lbrace 0, \dots, A\texttt{.length} - 1 \right\rbrace$}
            ...
            \For{$j = i+1$ to $n$ }
                 ...
                 \If{$A\left[i \right] < A\left[ min\right]$}
                    \State $min = j$
                \EndIf
            \EndFor
        \State $A[min] and\ A[i]$
